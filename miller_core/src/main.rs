...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if let Some(bg_path) = target_project_path {
        ...
        tokio::task::spawn(async move {
            ...
            let (changed_nodes, skipped_count) = scan_and_parse_project_incremental(&bg_path);
            
            if skipped_count > 0 { println!("[Background Worker] Fast skip operational: {} files unchanged.", skipped_count); }
            if changed_nodes.is_empty() { println!("\n[Background Worker] Cache clean. Workspace state sync complete."); return; }

            println!("[Background Worker] Syncing {} modified items into local Qdrant...", changed_nodes.len());
    ...
    if let Ok(query_vector) = get_ollama_embedding(&client, original_task).await {
        if let Ok(matched_chunks) = memory_layer.search_similar_code(query_vector, 2).await {
            if !matched_chunks.is_empty() {
                println!("[Retrieval Match] Valid structural matches found. Injecting blocks...");
    ...
    while attempts < MAX_ATTEMPTS {
        ...
        let ai_response = match generate_with_retry(&client, &current_prompt).await {
            Ok(text) => text,
            Err(e) => { println!("[Network Error] Ollama call permanently failed: {}", e); break; }
        };

        let code_to_write = match sanitize_code(&ai_response) {
            Some(code) => code,
            None => {
                println!("[Sanitizer] Code standard structure block not captured. Re-prompting...");
        ...
        println!("[Filesystem] Code successfully written to sandbox: '{}'", TARGET_FILE);
        ...
        match run_compiler_test() {
            Ok(true) => {
                ...
                match run_sandbox_execution() {
                    ...
                    Err(stderr) => {
                        println!("\n[Sandbox Runtime Error] Executable container crash.");
